...
@pytest.mark.asyncio
async def test_list_alerts(client: AsyncClient, admin_token: str, test_org: Organization, db: AsyncSession):
    await alert_crud.create(db, obj_in=AlertCreate(
...
@pytest.mark.asyncio
async def test_acknowledge_alert(client: AsyncClient, admin_token: str, test_org: Organization, db: AsyncSession):
...
@pytest.mark.asyncio
async def test_acknowledge_already_acknowledged(client: AsyncClient, admin_token: str, test_org: Organization, db: AsyncSession):
    a = await alert_crud.create(db, obj_in=AlertCreate(
        organization_id=test_org.id,
        raw_data={},
        timestamp=datetime.now(timezone.utc),
...
@pytest.mark.asyncio
async def test_link_alert_to_incident(client: AsyncClient, admin_token: str, test_org: Organization, db: AsyncSession):
    a = await alert_crud.create(db, obj_in=AlertCreate(
        organization_id=test_org.id,
        raw_data={},
        timestamp=datetime.now(timezone.utc),
...
@pytest.mark.asyncio
async def test_link_alert_twice(client: AsyncClient, admin_token: str, test_org: Organization, db: AsyncSession):
    a = await alert_crud.create(db, obj_in=AlertCreate(
        organization_id=test_org.id,
        raw_data={},
        timestamp=datetime.now(timezone.utc),
...
@pytest.mark.asyncio
async def test_student_cannot_create_alert(client: AsyncClient, student_token: str, test_org: Organization):
    resp = await client.post(
        "/api/v1/alerts",
        json={
            "organization_id": str(test_org.id),
